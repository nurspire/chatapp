...
export async function GET(req) {
  ...
  try {
    ...
    const matchedFriendships = acceptedData.filter(friendship => friendship.receiver_id === userId);
...
const matchedUsers = matchedFriendships.map(friendship => {
    ...
    const user = usersData.users.find(u => u.user_id === friendship.requester_id);
    
    // console.log("User found for requester_id:", friendship.requester_id, user);
  
    if (!user) {
      console.error(`No user found for requester_id: ${friendship.requester_id}`);
    }

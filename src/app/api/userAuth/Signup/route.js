...
export async function POST(request) {
  try {
    ...
    const verificationTokenExpires = Date.now() + 3600000; // 1 hour in milliseconds

    // Create a new user object with email, verification token, and expiry time
    const newUser = new User({
      email,
      verificationToken,
      verificationTokenExpires, // Add expiry time here
      isVerified: false,
    });

    // Save the user with the token and expiry in the database
    await newUser.save();
